...
//	var cache = {};
    ...
    var htmlEncode = function (str) { // Better than htmlEncode
...
//	}
    ...
    var genNodeCallback = function (markup, refNode) {
        ...
        if (Array.isArray(markup)) { // Array of values
            markup.forEach(function (_markup) {
                var cb = {}
                if (_markup && typeof _markup == 'object' && _markup.type == 'htmlCollection') { // Array of htmlCollection
                ...
                else if (_markup && typeof _markup == 'object' && _markup.type == 'templateObj') { // Array of templateobj values
                    var rootTemplateNode = createElement(_markup.templateStr);
                    rootTemplateNode = render(_markup, rootTemplateNode);
        ...
        else if (markup && typeof markup == 'object' && markup.type == 'templateObj') { // Templateobj values
        ...
        else if (markup && typeof markup == 'object' && (markup.nodeType == ELEMENT_NODE || markup.nodeType == DOCUMENT_FRAGMENT_NODE)) { // Dom value
            callback.newNode = markup;
        }
        else {
    ...
    var removeCommentsBlock = function (commentNode) {
        if (!(commentNode.previousSibling && commentNode.previousSibling.nodeType == COMMENT_NODE && commentNode.previousSibling.textContent == 'top')) {
            var prevNode = commentNode.previousSibling;
            while (prevNode && prevNode.textContent != 'top') { // Remove Childes until <!--top-->
    ...
    var domTemplate = function (selector, vars) {
        ...
        if (templateClone) {
            ...
            var templateString = htmlDecode(templateClone.innerHTML);
    ...
    var render = function (templateObj, selector) {
        ...
        if (templateObj && typeof templateObj == 'function') {
